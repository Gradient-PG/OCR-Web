...
async def create_task(
    ...
    try:
        task_id = await task_service.create_task('labeling', requests)
...
async def status(task_id: str, task_service: TaskService = Depends(get_task_service)):
    ...
    try:
        ...
        if task.task_type != 'labeling':
            raise HTTPException(status_code=400, detail='Task type is not labeling')
        return {'status': task.status}
    except Exception as e:
        raise HTTPException(status_code=404, detail=str(e)) from e

@router.get('/result/{task_id}', summary='Task result', description='Returns a task result')
async def result(task_id: str, task_service: TaskService = Depends(get_task_service)):
