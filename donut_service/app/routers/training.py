...
async def create_task(task_service: TaskService = Depends(get_task_service)):
    """Endpoint to create a new task."""
    try:
        task_id = await task_service.create_task('training', None)
...
async def status(task_id: str, task_service: TaskService = Depends(get_task_service)):
    ...
    try:
        ...
        if task.task_type != 'training':
            raise HTTPException(status_code=400, detail='Task type is not training')
